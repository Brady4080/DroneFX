...
public class TelloDrone extends MultiRotorDrone {
	...
	@Override
	public void flyForward(int pixels) throws IOException {
		int distanceInFeet = pixels / Constants.PIXELS_TO_ONE_MODEL_FOOT;
		// Calculate the distance in centimeters
		int front = distanceInFeet * Constants.CENTIMETERS_PER_MODEL_FOOT;
	...
	public void flyRCForward(double front) throws IOException {
		// Calculate the distance in feet from pixels
		double distanceInFeet = front / Constants.PIXELS_TO_ONE_MODEL_FOOT;
		// Calculate the distance in centimeters
		double distanceInCentimeters = distanceInFeet * Constants.CENTIMETERS_PER_MODEL_FOOT;

		System.out.println("Distance: " + distanceInCentimeters + " cm");

		this.controller.sendCommand("speed?");

		double time_to_wait = distanceInCentimeters / 100;
		double milliSec = time_to_wait * 1000;

		this.controller.sendCommand("rc 0 20 0 0");
		...
		// Send the RC command to stop
		this.controller.sendCommand("rc 0 0 0 0");

		long endTime = System.currentTimeMillis();

		long actualFlightTimeMs = endTime - startTime;
		System.out.println("Actual flight time: " + actualFlightTimeMs + " ms");
	}
